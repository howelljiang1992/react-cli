...
import {cleanBannerFileList} from '../actions/fileList'
import {setContent} from '../actions/content'
import delegate from '../utils/Delegate'
let actions = Object.assign({}, _actions, {cleanBannerFileList, setContent})
...
class Add extends Component{
    ...
    componentWillMount() {
        const {setContent, cleanBannerFileList} = this.props
        setContent('')
        cleanBannerFileList()
    }
    ...
    handleBack(e, href) {
        ...
        const modal = confirm({
            ...
            onOk() {
                ...
                else location.hash = '/' + location.hash.split('?')[0].replace(/(#|\/|add)/g, '')
            }
        })
        window.onpopstate = () => {
            modal.destroy()
    ...
    render() {
        ...
        const {result, banner} = this.props
...
function success() {
    ...
    setTimeout(() => {
        modal.destroy()
        location.hash = '/' + location.hash.split('?')[0].replace(/(#|\/|add)/g, '')
    }, 1000)
}
...
export default connect(mapStateToProps, mapDispatchToProps)(createForm()(Add))
